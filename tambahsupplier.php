...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nama_supplier) || empty($no_tlpn) || empty($alamat) || empty($email)) {
        echo "<script>alert('Harap isi semua field'); window.history.back();</script>";
    } else {
        // Query untuk menyimpan data baru
        $sql = "INSERT INTO supplier (nama_supplier, no_tlpn, alamat, email) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssss", $nama_supplier, $no_tlpn, $alamat, $email);

        if ($stmt->execute()) {
            echo "<script>alert('Data berhasil ditambahkan'); window.location.href = 'tables/supplier.php';</script>";
        } else {
            echo "<script>alert('Gagal menambahkan data'); window.location.href = 'tambahsuppplier.php';</script>";
        }
    }
}
?>
